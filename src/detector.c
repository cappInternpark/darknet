...
void validate_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou)
{
	...
	for (i = nthreads; i < m + nthreads; i += nthreads) {
		...
		/*** for image allocation to threads ***/
		for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
			...
			/*** above is same with test except for letterbox, 'int map' & 'int relative' of *dets ***/
			...
			/*** nboxes is # of boxes > thresh ***/
			for (i = 0; i < nboxes; ++i) {
				
				int class_id;
				for (class_id = 0; class_id < classes; ++class_id) {
					float prob = dets[i].prob[class_id]; // ex) prob of first class of first box
					if (prob > 0) {
						// write box inform. in detections
						detections_count++;
						...
						/*** thresh_calc_avg_iou = input thresh ***/
	...
	int rank;
	/*********************
	 *** MAKE PR-curve && Calc recall, precision
	 *** But PR[] isn't used
	 ***
	 **********************/
	...
	/*********************
	 *** In class i
	 *** cur_recall = 0 : 0.1 : 0.9
	 *** box.recall > cur_recall in detections
	 *** 	choose max precision
	 *** 	
	 *** 
	 *** avg = sum(max_precisions)/11
	***********************/
	for (i = 0; i < classes; ++i) {
...
void validate_detector_map_v2(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, int excel_format, int no_img)
{
	...
	for (i = nthreads; i < m + nthreads; i += nthreads) {
		...
		for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
			...
			int letterbox = (args.type == LETTERBOX_DATA);
			float hier_thresh = 0;
			// thread value is lower than test function, thresh = 0.005
			// this means we'll get all boxes in image
			detection *dets = get_network_boxes(&net, 1, 1, thresh, hier_thresh, 0, 0, &nboxes, letterbox);
			//detection *dets = get_network_boxes(&net, val[t].w, val[t].h, thresh, hier_thresh, 0, 1, &nboxes, letterbox); // for letterbox=1
			if (nms) do_nms_sort(dets, nboxes, l.classes, nms);
	...
	draw_box_width(im, left, top, right, bot, width, rgb[0], rgb[1], rgb[2]);	// ground truth
				}
			}
			// nboxes is # of boxes > thresh
			for (i = 0; i < nboxes; ++i) {
...
if(!no_img){
	...
	draw_box_width(im, left, top, right, bot, width, rgb[0], rgb[1], rgb[2]);	// tp
	draw_label(im, top + width, left, label, rgb);
	//cvPutText(im, class_name, CvPoint(left,top), 1, 1, (0,1,0));
}
...
if(!no_img){
	...
	draw_box_width(im, left, top, right, bot, width, rgb[0], rgb[1], rgb[2]);	// fp
	draw_label(im, bot - width + 6, left, label, rgb);
	//cvPutText(im, class_name, CvPoint(left,top), 1, 1, (0,0,1));
}
							}
							free_image(label);
						}
			...
			if(!no_img){
				char name_of_img[256];
				char *tmp = basename(path);
				tmp[strlen(tmp)-4] = '\0';
				sprintf(name_of_img,"results_map/predict_%s",tmp);
				save_image(im, name_of_img);
			}
			unique_truth_count += num_labels;
	...
	if(excel_format){
		...
		if(size == 0){
			fprintf(fw,"iter\tcur_precision\tcur_recall\tf1_score\t");
				   ...
				   float hier_thresh, int dont_show, int ext_output, int save_labels)
{
    ...
    while(1){
		...
		char name_of_img[256];
		char *tmp = basename(input);
		tmp[strlen(tmp)-4] = '\0';
		sprintf(name_of_img,"results/predict_%s",tmp);
		save_image(im, name_of_img);
...
void run_detector(int argc, char **argv)
{
	...
	else if(0==strcmp(argv[2], "map_v2")) validate_detector_map_v2(datacfg, cfg, weights, thresh, excel_format, no_img);
	else if(0==strcmp(argv[2], "map_v3")) {
		list *options = read_data_cfg(datacfg);

		network net = parse_network_cfg_custom(cfg, 1);	// set batch=1
		if (weights) {
			load_weights(&net, weights);
		}
		validate_detector_map_v3(options, net, weights, thresh, 0, 1, 1);
	}
